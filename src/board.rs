...
impl Board {
    ...
    pub fn reset(&mut self) {
        self.board = vec![Tile::Empty; CONFIG.get().expect(CONFIG_ERROR).total_tiles];
    }
    ...
    pub fn from_input(&self, input: Input) -> usize {
        input.0 + input.1 * CONFIG.get().expect(CONFIG_ERROR).board_length
    }

    pub fn get_input(&self, i: usize) -> Input {
        (i % CONFIG.get().expect(CONFIG_ERROR).board_length, i / CONFIG.get().expect(CONFIG_ERROR).board_length)
    }

    fn replace(&mut self, input: Input, tile: Tile) {
        let size = CONFIG.get().expect(CONFIG_ERROR).board_length;
        self.board[input.0 + input.1 * size] = tile;
    }

    pub fn get(&self, input: Input) -> Tile {
        self.board[input.0 + input.1 * CONFIG.get().expect(CONFIG_ERROR).board_length]
    }

    pub fn get_protected(&self, input: Input) -> Option<&Tile> {
        self.board.get(input.0 + input.1 * CONFIG.get().expect(CONFIG_ERROR).board_length)
    }

    pub fn get_ref(&self, input: Input) -> &Tile {
        self.board.get(input.0 + input.1 * CONFIG.get().expect(CONFIG_ERROR).board_length).unwrap()
    }
    ...
    pub fn check_add_value(&self, input: Input, players: &Players) -> Result<(), PlacementError> {
        if input.0 > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || input.1 > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 {
    ...
    pub fn add_value_checked(&mut self, input: Input, players: &mut Players) {
        ...
        if CONFIG.get().expect(CONFIG_ERROR).capture_range != 0 {
    ...
    fn slice(&self, start: usize, length: usize, f_x: fn(usize, usize) -> usize, f_y: fn(usize, usize) -> usize) -> Vec<Tile> {
        ...
        for i in 0..length {
            ret.push(self.board[f_x(start, i) + f_y(start, i) * CONFIG.get().expect(CONFIG_ERROR).board_length])
        }
        ret
    }

    fn check_victory(&self, i: usize, color: Color, tile: &Tile, captured: bool) -> (bool, Option<Color>) {
        let is_capture_disabled: bool = CONFIG.get().expect(CONFIG_ERROR).capture_range == 0;
        if  i % CONFIG.get().expect(CONFIG_ERROR).board_length <= CONFIG.get().expect(CONFIG_ERROR).board_length - CONFIG.get().expect(CONFIG_ERROR).alignement_nb &&
            self.board[i..i + CONFIG.get().expect(CONFIG_ERROR).alignement_nb].iter().all(|x| *x == *tile) &&
        ...
        if  i / CONFIG.get().expect(CONFIG_ERROR).board_length <= CONFIG.get().expect(CONFIG_ERROR).board_length - CONFIG.get().expect(CONFIG_ERROR).alignement_nb &&
            self.slice(i, CONFIG.get().expect(CONFIG_ERROR).alignement_nb, |start, _| start, |_, x| x).iter().all(|x| *x == *tile) &&
            (is_capture_disabled || (captured || cannot_be_captured(self, self.get_input(i), color, |x, _| x , |x, y| (x as i32 + y) as usize))) {
            return (true, Some(color))
        }
        if  i / CONFIG.get().expect(CONFIG_ERROR).board_length <= CONFIG.get().expect(CONFIG_ERROR).board_length - CONFIG.get().expect(CONFIG_ERROR).alignement_nb &&
            i % CONFIG.get().expect(CONFIG_ERROR).board_length <= CONFIG.get().expect(CONFIG_ERROR).board_length - CONFIG.get().expect(CONFIG_ERROR).alignement_nb &&
            self.slice(i, CONFIG.get().expect(CONFIG_ERROR).alignement_nb, |start, x| start + x, |_, x| x).iter().all(|x| *x == *tile) &&
        ...
        if  i / CONFIG.get().expect(CONFIG_ERROR).board_length <= CONFIG.get().expect(CONFIG_ERROR).board_length - CONFIG.get().expect(CONFIG_ERROR).alignement_nb &&
            i % CONFIG.get().expect(CONFIG_ERROR).board_length >= CONFIG.get().expect(CONFIG_ERROR).alignement_nb - 1 &&
            self.slice(i, CONFIG.get().expect(CONFIG_ERROR).alignement_nb, |start, x| start - x, |_, x| x).iter().all(|x| *x == *tile) &&
            (is_capture_disabled || (captured || cannot_be_captured(self, self.get_input(i), color, |x, y| (x as i32 - y) as usize, |x, y| (x as i32 + y) as usize))) {
            return (true, Some(color))
        }
...
impl fmt::Display for Board {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let legend: String = (0..CONFIG.get().expect(CONFIG_ERROR).board_length)
            .collect::<Vec<usize>>()
            .iter()
            .map(|x| *x % 10)
            ...
            .to_string()
            + "\n0 ";
        let total_tiles_minus_1: usize = CONFIG.get().expect(CONFIG_ERROR).total_tiles- 1;
        let my_str: String = self.board.iter().enumerate().fold(legend, |acc, (i, x)| {
            if i == total_tiles_minus_1 {
                return format!("{}{}", acc, x);
            } else if (i + 1) % CONFIG.get().expect(CONFIG_ERROR).board_length == 0 {
                return format!("{}{}\n{} ", acc, x, ((i / CONFIG.get().expect(CONFIG_ERROR).board_length) + 1) % 10);
            } else {
                return format!("{}{}  ", acc, x);
            }
        });
...
fn cannot_be_captured(
    ...
    for i in 0..CONFIG.get().expect(CONFIG_ERROR).alignement_nb {
        if f_x(input.0, i as i32) < CONFIG.get().expect(CONFIG_ERROR).board_length && f_y(input.1, i as i32) < CONFIG.get().expect(CONFIG_ERROR).board_length {
...
fn cannot_be_captured_prime(
    ...
    for i in 1..=CONFIG.get().expect(CONFIG_ERROR).capture_range {
        if f_x(input.0, i as i32) >= CONFIG.get().expect(CONFIG_ERROR).board_length || f_y(input.1, i as i32) >= CONFIG.get().expect(CONFIG_ERROR).board_length {
            break;
        }
    ...
    vec.reverse();
    for i in 1..=CONFIG.get().expect(CONFIG_ERROR).capture_range {
        if f_x(input.0, -(i as i32)) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, -(i as i32)) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 {
            break;
        }
        let tile = board.get((f_x(input.0, -(i as i32)), f_y(input.1, -(i as i32))));
        if tile == Tile::Color(color.get_inverse_color()) || tile == Tile::Empty {
    ...
    ) == (CONFIG.get().expect(CONFIG_ERROR).capture_range, true) {
        return false;
    } else {
        return true
    }
...
fn execute_capture(
    ...
    if f_x(input.0, (CONFIG.get().expect(CONFIG_ERROR).capture_range + 1) as i32) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, (CONFIG.get().expect(CONFIG_ERROR).capture_range + 1) as i32) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 {
        return
    }
    let mut selected_vec: Vec<Tile> = Vec::with_capacity(CONFIG.get().expect(CONFIG_ERROR).capture_range + 1);
    let color = players.get_current_player().get_player_color();
    for i in 1..=CONFIG.get().expect(CONFIG_ERROR).capture_range + 1 {
    ...
    match (
        ...
        (true, true) => {
            for z in 1..=CONFIG.get().expect(CONFIG_ERROR).capture_range {
...
fn case1(board: &Board, input: Input, color: Color, f_x: fn(usize, i32) -> usize, f_y: fn(usize, i32) -> usize) -> u8 {
    if f_x(input.0, 3) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_x(input.0, -1) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, 3) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, -1) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 {
        return 0
    }
    match (
        color,
...
fn case2(board: &Board, input: Input, color: Color, f_x: fn(usize, i32) -> usize, f_y: fn(usize, i32) -> usize) -> u8 {
    if f_x(input.0, 2) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_x(input.0, -2) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, 2) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, -2) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 {
        return 0
    }
    match (
        color,
...
fn _case3(board: &Board, input: Input, color: Color, f_x: fn(usize, i32) -> usize, f_y: fn(usize, i32) -> usize) -> u8 {
    if f_x(input.0, 4) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_x(input.0, -1) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, 4) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, -1) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 {
        return 0
    }
    match (
        color,
...
fn case4(board: &Board, input: Input, color: Color, f_x: fn(usize, i32) -> usize, f_y: fn(usize, i32) -> usize) -> u8 {
    if f_x(input.0, 3) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_x(input.0, -2) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, 3) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 || f_y(input.1, -2) > CONFIG.get().expect(CONFIG_ERROR).board_length - 1 {
        return 0
    }
    match (
        color,
